...
let oss = {
  ...
  files: (localEnv, dir) => {
    ...
    return Helper.walk(root, false, {white: localEnv.whitelist, black: localEnv.ignore}).filter(item => {
      ...
      return !oss.isIgnored(localEnv, file)
    })
  },
...
module.exports = {
  deploy: (localEnv, dir, force, removePrefix) => {
    let files = oss.files(localEnv, dir)
    ...
    if (force) {
      oss.syncFiles(localEnv, files, removePrefix)
    } else {
